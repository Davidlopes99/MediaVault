...
import styles from './SidebarSeries.module.css';

const classifications = ['L', '10', '12', '14', '16', '18'];

type SidebarSeriesProps = {
  sortBy: string;
...
const SidebarSeries: React.FC<SidebarSeriesProps> = ({
  sortBy, setSortBy,
  ...
  return (
    <Form className={`${styles.sidebar} mb-4`}>
      {/* Ordenar por */}
      <Form.Group controlId="sortSelect" className={`${styles.section} mb-4`}>
        ...
        <div>
          {genres.map(g => (
            <Button
              ...
              size="sm"
              className={`${styles.buttonOutline} ${localSelectedGenre === String(g.id) ? styles.buttonSelected : ''}`}
              onClick={() =>
        ...
        <div>
          {classifications.map(c => (
            <Button
              ...
              size="sm"
              className={`${styles.buttonOutline} ${localClassification === c ? styles.buttonSelected : ''}`}
      ...
      {/* Onde assistir */}
      <Form.Group controlId="providerSelect" className={`${styles.section} mb-4`}>
      ...
      {/* País de produção */}
      <Form.Group controlId="countrySelect" className={`${styles.section} mb-4`}>
      ...
      {/* Busca por nome */}
      <Form.Group controlId="searchInput" className={`${styles.section} mb-4`}>
        <Form.Label className={styles.label}>Buscar por nome:</Form.Label>
        <Form.Control
          type="text"
          placeholder="Exemplo: Dexter"
      ...
      <div className={`${styles.section} mb-4`}>
        <Form.Label className={styles.sliderLabel}>Pontuação: {localMinRating} – {localMaxRating}</Form.Label>
        <div className="px-2">
          <Slider
            range
            min={0}
